...
def solve():
    ...
    for n in nodes_data:
        ...
        # Split into chunks of max 3000 weight and 13.5 volume (Type 1 capacity as baseline for FTL)
        # Actually, let's just split into chunks that can fit in Type 1 or 4
        # Max capacity across all vehicles: W=3000, V=13.5 (Type 1 is 13.5, Type 4 is 15. We use 13.5 so it fits both!)
        while rem_w > 0 or rem_v > 0:
            # Fraction based on max capacities
            f_w = min(1.0, 3000.0 / rem_w) if rem_w > 0 else 1.0
            f_v = min(1.0, 13.5 / rem_v) if rem_v > 0 else 1.0
            f = min(f_w, f_v)
    ...
    # Preference: Type 4 (Elec 3000), Type 1 (Fuel 3000), Type 5 (Elec 1250), Type 2 (Fuel 1500), Type 3 (Fuel 1250)
    v_preference = [4, 1, 5, 2, 3] 
    ...
    with open('问题1/solution_opt.json', 'w', encoding='utf-8') as f:
